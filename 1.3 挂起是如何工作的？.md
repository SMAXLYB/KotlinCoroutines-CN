...
```
这和之前的游戏类比有点不同，我不知道有哪一款游戏，可以在保存点恢复时放入一些东西到游戏的某个地方(除非你作弊去搜索下一个挑战，否则这都算新知识)。不过这对协程来说是讲得通的，在等待数据的时候我们经常被挂起，例如，我们从api中获取网络响应，这是个很常见的场景。你的线程执行业务逻辑直到它到达某个点需要去获取数据，接着它会让你的网络库交付这些数据，如果没用协程，这个线程就要停下来等待，这是个巨大的浪费——线程是非常昂贵的，特别是一个非常重要的线程，例如Android中的主线程。如果用协程，它(协程)只是挂起， 
